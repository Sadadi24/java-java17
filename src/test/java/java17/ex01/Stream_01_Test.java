...
public class Stream_01_Test {
	...
	public void test_stream_filter_collect_counting() throws Exception {
		...
		long result = pizzas.stream().filter(p->p.getPrice()>=1300).toList().size();
...
;
	...
	public void test_stream_filter_adv() throws Exception {
		...
		// pizza Pépéroni
		List<Order> result = orders.stream().filter(o-> "Pépéroni".equals(o.getCustomer().getGender().name())).toList();;
